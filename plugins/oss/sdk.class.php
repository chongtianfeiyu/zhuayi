...
//加载conf.inc.php文件
require OSS_API_PATH.DIRECTORY_SEPARATOR.'conf.inc.php';

//加载RequestCore
require OSS_API_PATH.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'requestcore'.DIRECTORY_SEPARATOR.'requestcore.class.php';

//加载MimeTypes
require OSS_API_PATH.DIRECTORY_SEPARATOR.'util'.DIRECTORY_SEPARATOR.'mimetypes.class.php';
...
class ALIOSS{
	 ...
	 * @return ResponseCore 
	 */
	public function create_mtu_object_by_dir($bucket,$dir,$recursive = false,$exclude = ".|..|.svn|.DS_Store",$options = null){
		//options
		$this->validate_options($options);
		...
		$file_list_array = $this->read_dir($dir,$exclude,$recursive,$dir);
	 ...
	 * @return array
	 */
	private  function read_dir($dir,$exclude = ".|..|.svn",$recursive = false,$original_dir){
		static $file_list_array = array();
		$exclude_array = explode("|", $exclude);

		if (substr($original_dir,-1,1) != '/')
		{
			$original_dir .= "/";
		}
		//读取目录
			...
			while ( false !== ($file = readdir($handle))){						
				if(!in_array(strtolower($file),$exclude_array)){
					...
					if(is_dir($new_file) && $recursive){
						$this->read_dir($new_file,$exclude,$recursive,$original_dir);
					}else{
						$file_list_array[] = array(
							'path' => $new_file,
							'file' => str_replace($original_dir,'',$new_file),
						);
					}
				}
			}
